...
func Test_isSSLRequest_With_ValidPacket_Returns_True(t *testing.T) {
	packet := Packet{decodeHexStream(t, "0000000804d2162f"), OriginFrontend}
...
func Test_isSSLRequest_With_InvalidPacket_Returns_False(t *testing.T) {
	...
	for _, invalidPacket := range invalidPackets {
		packet := Packet{decodeHexStream(t, invalidPacket), OriginFrontend}
		if isSSLRequestMessage(packet.Payload) {
...
func Test_isStartupMessage_With_ValidPacket_Returns_True(t *testing.T) {
	packet := Packet{decodeHexStream(t, "0000007000030000757365720079615f74657374696e6700646174616261736500706f73746772657300636c69656e745f656e636f64696e67005554463800446174655374796c650049534f0054696d655a6f6e65005554430065787472615f666c6f61745f64696769747300320000"), OriginFrontend}
...
func Test_Build_With_ValidPacketChunks_Returns_Nil(t *testing.T) {
	...
	for _, chunk := range chunks {
		packet, _ = builder.Build(decodeHexStream(t, chunk), OriginFrontend)
	}
	if packet == nil {
